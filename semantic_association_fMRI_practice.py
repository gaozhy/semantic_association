...
num_trials = 24 # total trial numbers of each run 
...
def info_gui(expName):
    ...
    dlg = gui.DlgFromDict(expInfo,title='input data', fixed = ['expname','expdate'],order =['expname','expdate','subjID','subjName'])
...
# creates a file with a name that is absolute path + info collected from GUI
    filename = data_folder + os.sep + '%s_%s_%s_prac.csv' %(expInfo['subjID'], expInfo['subjName'], expInfo['expdate'])
